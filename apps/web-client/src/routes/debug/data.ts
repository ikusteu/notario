import type { DatabaseInterface } from "@notario/db";
import type { ProjectInterface } from "@notario/db/dist/project";
import type { SectionInterface } from "@notario/db/dist/section";
...
const notes = [
	...
	},
	{
		...
		resourceURL: "resource-5"
	}
];

export const loadNotes = async (db: DatabaseInterface) => {
	for (const { id, content, resourceURL, resourceName } of notes) {
		await db
			.resource()
			.create()
			.then((r) => r.setPath(resourceURL))
			.then((r) => r.setName(resourceName))
			.then((r) => r.note(id).create())
			.then((n) => n.setContent(content));
	}
};
// #endregion resources
...
const loadSubections = async (section: SectionInterface, subsections: ReturnType<typeof createSubsections>) => {
	for (const { id, name, notes } of subsections) {
		await section
			.subsection(id)
			.create()
			.then((sub) => sub.setName(name))
			.then((sub) => sub.setNotes(notes));
	}
};

const loadSections = async (project: ProjectInterface) => {
	for (const { id, name, notes, subsections } of sections) {
		await project
			.section(id)
			.create()
			.then((s) => s.setName(name))
			.then((s) => s.setNotes(notes.map(({ id }) => id)))
			.then((s) => loadSubections(s, subsections));
	}
};

export const loadProject = (db: DatabaseInterface) =>
	db
		.project("project-1")
		.create()
		.then((p) => p.setName("Project 1"))
		.then(loadSections);
// #endregion project
